...
ggplot(data = data, aes(x = as.factor(volcano_name), y = n)) + 
  geom_bar(stat = "identity", fill = "#F0E442") + theme_minimal() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  ggtitle("Most active volcanoes") + xlab("Volcano") + ylab("Confirmed erruptions")
...
volcano %>% filter(volcano_name %in% top10) %>% select(volcano_name, primary_volcano_type, last_eruption_year,
  ...
  gt() %>%
  tab_header(
    title = html("<h3>10 most active volcanoes</h3>")
  )  %>%
  ...
  tab_style(
    style = list(
      cell_fill(color = "gold", alpha = 0.5)
    ), 
